...
# FIXES:
- [ ] при игре на 4 игрока a,b,c,d у последнего игрока команды, который скидывается на проект не отображается построено ли здание, а сразу же начинается следующий раунд. У всех остальных при этом есть всплывающее окно "здание построено/не построено"
# FEATURES:
...
# TESTS:
- [x] [unit tests](https://www.dataquest.io/blog/unit-tests-python/#:~:text=Unit%20tests%20are%20segments%20of,and%20catch%20bugs%20early%20on)
- [ ] написать тесты для функций, которые не используют бд и сокеты
apply_effects, cancel_effects, populate_...,
